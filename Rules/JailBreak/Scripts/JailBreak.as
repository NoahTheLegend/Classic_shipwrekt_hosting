...
string cost_config_file = "jb_vars.cfg";
...
void Config(TDMCore@ this)
{
	...
	this.minimum_players_in_team = 2;
...
shared class TDMSpawns : RespawnSystem
{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			p_info.blob_name = (p_info.team == 0 ? "knight" : "builder");
	...
	Vec2f getSpawnLocation(PlayerInfo@ p_info)
	{
		...
		if(s32(p_info.team) == 0)
		{
			...
			Vec2f respawnPos = respawnPositions[XORRandom(997) % respawnPositions.length];
			respawnPos.y -= 16.0f;
			return respawnPos;
		}

		// team 1
		if(s32(p_info.team) == 1)
		{
			Vec2f[] respawnPositions;
...
shared class TDMCore : RulesCore
{
	...
	void CheckTeamWon()
	{
		...
		if (winteamIndex >= 0)
		{
			...
			rules.SetCurrentState(GAME_OVER);
			rules.SetGlobalMessage("Guardians win the game!");
